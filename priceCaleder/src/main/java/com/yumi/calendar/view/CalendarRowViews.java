 ...
 */
public class CalendarRowViews extends ViewGroup implements View.OnClickListener {
    private boolean isHeaderRow;
    private MonthViews.Listener listener;

    public CalendarRowViews(Context context, AttributeSet attrs) {
        super(context, attrs);
    }
    ...
    public void setListener(MonthViews.Listener listener) {
        this.listener = listener;
    }

    public void setDayViewAdapter(DayViewAdapter adapter) {
        for (int i = 0; i < getChildCount(); i++) {
            if (getChildAt(i) instanceof CalendarCellViews) {
                CalendarCellViews cell = ((CalendarCellViews) getChildAt(i));
                cell.removeAllViews();
    ...
    public void setCellTextColor(int resId) {
        for (int i = 0; i < getChildCount(); i++) {
            if (getChildAt(i) instanceof CalendarCellViews) {
                ((CalendarCellViews) getChildAt(i)).getDayOfMonthTextView().setTextColor(resId);
            } else {
    ...
    public void setCellTextColor(ColorStateList colors) {
        for (int i = 0; i < getChildCount(); i++) {
            if (getChildAt(i) instanceof CalendarCellViews) {
                ((CalendarCellViews) getChildAt(i)).getDayOfMonthTextView().setTextColor(colors);
            } else {
    ...
    public void setTypeface(Typeface typeface) {
        for (int i = 0; i < getChildCount(); i++) {
            if (getChildAt(i) instanceof CalendarCellViews) {
                ((CalendarCellViews) getChildAt(i)).getDayOfMonthTextView().setTypeface(typeface);
            } else {
