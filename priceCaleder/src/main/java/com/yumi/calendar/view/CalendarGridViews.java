 ...
 */
public class CalendarGridViews extends ViewGroup {
    /**
    ...
    public CalendarGridViews(Context context, AttributeSet attrs) {
        super(context, attrs);
    ...
    public void setDayViewAdapter(DayViewAdapter adapter) {
        for (int i = 0; i < getChildCount(); i++) {
            ((CalendarRowViews) getChildAt(i)).setDayViewAdapter(adapter);
        }
    }

    public void setDayBackground(int resId) {
        for (int i = 1; i < getChildCount(); i++) {
            ((CalendarRowViews) getChildAt(i)).setCellBackground(resId);
        }
    }
    ...
    public void setDayTextColor(int resId) {
        for (int i = 0; i < getChildCount(); i++) {
            ...
            ((CalendarRowViews) getChildAt(i)).setCellTextColor(colors);
        }
    }
    ...
    public void setHeaderTextColor(int color) {
        ((CalendarRowViews) getChildAt(0)).setCellTextColor(color);
    }

    public void setTypeface(Typeface typeface) {
        for (int i = 0; i < getChildCount(); i++) {
            ((CalendarRowViews) getChildAt(i)).setTypeface(typeface);
        }
    }
    ...
    public void addView(View child, int index, LayoutParams params) {
        if (getChildCount() == 0) {
            ((CalendarRowViews) child).setIsHeaderRow(true);
        }
